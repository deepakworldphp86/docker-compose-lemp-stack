...
echo "Configuring Redis object cache: /var/www/html/bin/magento setup:config:set --cache-backend=redis ${redis_obj_args[@]}"
/var/www/html/bin/magento setup:config:set --cache-backend=redis "${redis_obj_args[@]}"
